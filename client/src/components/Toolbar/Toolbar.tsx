...
export function Toolbar({ onReferenceImageChange, hasReferenceImage }: ToolbarProps) {
  const { project, setTool, setBrushSize, setShapeMode, setBorderRadius, setZoom, setMoveAllLayers } = useEditorStore();
  const [isRefModalOpen, setIsRefModalOpen] = useState(false);

  if (!project) return null;

  const { selectedTool, brushSize, shapeMode, borderRadius, zoom, moveAllLayers } = project.uiState;
  ...
  return (
    <div className="toolbar">
      ...
      {selectedTool === 'reference-trace' && hasReferenceImage && (
        <div className="toolbar-section trace-hint">
          <span className="trace-hint-text">🎯 WASD to align • Click to copy pixel</span>
        </div>
      )}
