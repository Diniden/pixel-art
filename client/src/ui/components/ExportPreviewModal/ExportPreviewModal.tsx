...
interface ExportPreviewModalProps {
  ...
  kebabName: string;
  /**
   * Fetches the exported `frames.json` for `kebabName`.
   *
   * REFRESH task 36 (W27): this component used to call `exportApi` directly,
   * which is the one thing that kept it out of `ui/`. The call is now injected
   * by `ExportPreviewModalContainer`, so the component itself is pure — it
   * still owns the *when* (the open/abort lifecycle below is unchanged), the
   * container owns the *how*.
   *
   * The `signal` is forwarded so the existing abort-on-close semantics survive
   * the move; a container that ignores it reintroduces the late-setState the
   * `cancelled` flag guards against.
   */
  loadExport: (kebabName: string, signal: AbortSignal) => Promise<unknown>;
}
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Task 15 routed this through the typed API layer (timeout + abort).
        // Task 36 turned that call into the `loadExport` prop so the component
        // holds no API import; the container still hands it `exportApi`.
        const raw = await loadExport(kebabName, controller.signal);
