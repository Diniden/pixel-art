...
**Current position:** COMPLETE (2026-08-29)
...
**Last commit:** `0513820` (+ this ledger commit)
...
| W3 | 06 | DONE | 2026-08-29 | `0513820` | tsc client+server clean · eslint 0 errors/64 warnings · vitest 117 files / 1984 tests pass (corpus digests unchanged) · lint:boundaries OK · stylelint 2 errors = pre-existing OtherHand.css · storybook `✓ built in 5.42s` · **root `bun run verify` exit 0** (re-run by coordinator: exit 0, 117/1984) · no lockfile. 11/11 manual checks reported (see notes). |
...
- **W3 manual checklist (task 06), headless Chrome against a scratchpad copy of the server (real `server/src/data/` never touched):** 1 fresh load PASS · 2 open Layer view PASS · 3 cross-pane draw + single undo PASS · 4 independent cameras PASS (mouse; pinch NOT performed) · 5 swap = same DOM nodes, no remount PASS · 6 arrows/WASD exactly once with two panes, shift = all frames PASS · 7 close PASS · 8 reload → single Full pane, Full camera restored PASS · 9 non-variant layer → object grid, arrows gone PASS · 10 focus mode PASS, portrait emulated PASS (real iPad NOT performed) · 11 no console errors / no doubled keys PASS.
- **`bun run dev` (mprocs) could not be exercised headlessly** (needs a TTY); `dev:client` + the server entry both start. Owner should run `bun run dev` once.
- **Still owed to a human:** touch/pinch per pane, iPad portrait stacking, and an eyes-on visual pass (buttons square/equal, split divider, storybook stories for CanvasViewControls/CanvasSplit).
